...
def index(request):
    ...
    try:
        ...
        log.error('https is not a registered namespace', 'Request from {}'.format(domain),
                  request.site.__dict__,
                  'enable_mktg_site= ' + str(enable_mktg_site),
                  'Auth Status= ' + str(request.user.is_authenticated)
                  )
        raise
